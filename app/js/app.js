...
import '~/app/js/jquery.fancybox.min.js'
...
document.addEventListener('DOMContentLoaded', () => {
	...
	$('.swiper__suggestion__slide').each(function(){
		var slider=$(this);

		swiperSuggestion = new Swiper(slider[0], {
			slidesPerView: "auto",
			spaceBetween: 20,
			loop: false,
			pagination: true,
			...
			navigation: {
				nextEl: slider.find('.swiper-button-next__suggestion')[0],
				prevEl:  slider.find('.swiper-button-prev__suggestion')[0]
			},
			breakpoints: {
	...
	var seriesProducts;
	$('.seriesProducts__slide').each(function(){
		var slider=$(this);

		seriesProducts = new Swiper(slider[0], {
			slidesPerView: "auto",
			spaceBetween: 20,
			loop: false,
			pagination: true,
...
jQuery(function ($) {
	...
	$('.js-finput input[type="file"]').on('change', function () {
		var finput=$(this).closest('.finput');
		if (typeof ($(this)[0].files) !== 'undefined' && typeof ($(this)[0].files[0]) !== 'undefined') {
			finput.find('.finput__value').html($(this)[0].files[0].name);
			finput.addClass('finput--file-is-set');
		} else {
			finput.find('.finput__value').html('');
			finput.removeClass('finput--file-is-set');
		}
...
/* код в additional
	...
	$('.js-main-modal, a[href="#js-main-modal"], .js-main-modal-btn-title, a[href="#js-main-modal-title-from-btn"]').on('click', function (e) {
		e.preventDefault();
		e.stopPropagation();
		var btn=$(this);
		...
		if (isdef(dataTitle) && dataTitle === 'btn' || btn.hasClass('.js-main-modal-btn-title') || btn.attr('href')==='#js-main-modal-title-from-btn') {
			title = btn.text();
		}
		...
		$('#main-modal .mmodal-title').html(title);
		$('#main-modal form .js-hidden-textarea').val(target);
...
$(window).on('load',function (){
	if ($('.home-hero').length > 0){
		...
		let left_arc = $("#hero-ark--left").contents().find('path');
		let right_arc = $("#hero-ark--right").contents().find('path');

		left_arc.css({'stroke-dasharray': 217});
		right_arc.css({'stroke-dasharray': 217});
		...
		function recalc_hero(){
			let ww = $(window).width();
			let elem = $('.home-hero__center');
			if (ww < 1200) {
				let scale = ww/1400;
